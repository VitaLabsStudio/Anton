...
Archetype selection logic is robust. Redis usage for rotation is appropriate.

### Improvements Checklist

- [ ] **Fallback:** Ensure `selectFromPool` handles the unlikely case where filtering removes ALL archetypes (e.g., tiny pool + strict rotation). Add a fallback to `pool[0]` or `random` if empty.
...
Gate: PASS → docs/qa/gates/2.10-archetype-selector.yml
