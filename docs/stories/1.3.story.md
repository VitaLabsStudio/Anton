...
- [x] **Task 2: Define Prisma Enums (Static Only)** (AC: 10, TECH-002 Mitigation)
  ...
  - [x] **CONVERT to TABLES** (evolvable business logic - see Task 2a):
    - [x] ~~Archetype~~ → Table (8 initial values)
    - [x] ~~PowerTier~~ → Table (MICRO, MACRO, MEGA)
    - [x] ~~CompetitorCategory~~ → Table
    - [x] ~~KpiType~~ → Table
    - [x] ~~EscalationReason~~ → Table
...
- Coding Standards: ✓
- Project Structure: ✓
- Testing Strategy: ⚠ (Constraint tests exist but fail to run due to env config)
- All ACs Met: ✓ (Schema is correct and verified by `test-migration.sh`)

### Improvements Checklist

- [x] **CRITICAL**: Refactored `schema.prisma` to fix missing `onDelete` behaviors.
- [ ] **HIGH**: Fix `database/tests/constraints.test.ts` execution. The test runner is not picking up the correct `DATABASE_URL` (localhost vs docker service name). This prevents automated verification of constraints.
- [ ] Update `package.json` to include the missing `validate:schema` script mentioned in the story.
...
Gate: CONCERNS → docs/qa/gates/1.3-postgresql-database-schema-migrations.yml
Risk profile: docs/qa/assessments/1.3-risk-20251204.md
NFR assessment: docs/qa/assessments/1.3-nfr-20251204.md

### Recommended Status

[✓ Ready for Done] ( Conditional on test config fix )
The schema itself is solid and verified via migration tests. The remaining issue is purely test configuration. I am marking this as **CONCERNS** but allowing progress if the test config is addressed immediately.
