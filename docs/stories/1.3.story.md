...
- [ ] **Task 2: Define Prisma Enums (Static Only)** (AC: 10, TECH-002 Mitigation)
  - [ ] **KEEP as ENUMs** (truly static values):
    - [ ] Define Platform enum (TWITTER, REDDIT, THREADS)
    - [ ] Define OperationalMode enum (HELPFUL, ENGAGEMENT, HYBRID, DISENGAGED)
    - [ ] Define ApprovalStatus enum (PENDING, APPROVED, REJECTED)
    - [ ] Define ReplyType enum (MANUAL, AUTO)
    - [ ] Define ExperimentStatus enum (DRAFT, RUNNING, COMPLETED, CANCELLED)
    - [ ] Define EscalationPriority and EscalationStatus enums
  - [ ] **CONVERT to TABLES** (evolvable business logic - see Task 2a):
    - [ ] ~~Archetype~~ → Table (8 initial values)
    - [ ] ~~PowerTier~~ → Table (MICRO, MACRO, MEGA)
    - [ ] ~~CompetitorCategory~~ → Table
    - [ ] ~~KpiType~~ → Table
    - [ ] ~~EscalationReason~~ → Table

- [ ] **Task 2a: Create Evolvable Reference Tables** (TECH-002 CRITICAL Mitigation)
  - [ ] Create `Archetype` model (id, name, description, isActive) with 8 initial values
  - [ ] Create `PowerTier` model (id, name, followerThreshold, description)
  - [ ] Create `CompetitorCategory` model (id, name, description)
  - [ ] Create `KpiType` model (id, name, description, aggregationType)
  - [ ] Create `EscalationReason` model (id, name, description, severity)
  - [ ] Create `ChampionTier` model (id, name, minEngagement, benefits)
  - [ ] Update all relations to use foreign keys instead of enums
  - [ ] Create seed data for all reference tables
...
- [ ] **Task 15: Implement Production Secret Management** (SEC-001 CRITICAL Mitigation)
  - [ ] Choose secret management solution (Doppler recommended)
  - [ ] Install and configure secret manager
  - [ ] Migrate DATABASE_URL to secret manager
  - [ ] Update Docker Compose to use secret manager
  - [ ] Document secret management workflow in README
  - [ ] Add secret rotation procedures to documentation

- [ ] **Task 16: Add Secret Scanning to CI/CD** (SEC-001 Mitigation)
  - [ ] Create `.github/workflows/secret-scan.yml` (or equivalent)
  - [ ] Configure gitleaks or equivalent scanner
  - [ ] Test scanner with fake secrets
  - [ ] Document bypass procedure for false positives

- [ ] **Task 17: Create Secure Database Connection Wrapper** (SEC-001 Mitigation)
  - [ ] Create `database/prisma/connection-security.ts`
  - [ ] Add DATABASE_URL validation
  - [ ] Add production security checks (no localhost, SSL required)
  - [ ] Add secure logging (never log connection strings)
  - [ ] Export secure prisma client from shared package

- [ ] **Task 18: Configure Database Server Security** (SEC-001 Mitigation)
  - [ ] Create restricted database user (not superuser)
  - [ ] Configure pg_hba.conf for IP-based access control
  - [ ] Document database user creation in README
  - [ ] Test connection with restricted user
  - [ ] Verify superuser cannot be used by application

- [ ] **Task 19: Create Schema Validation Script** (TECH-001 Mitigation)
  - [ ] Create `scripts/validate-schema.ts`
  - [ ] Add check: All models have @@map directive
  - [ ] Add check: All relations have onDelete behavior
  - [ ] Add check: DateTime fields have defaults
  - [ ] Add check: Critical indexes exist
  - [ ] Add validation to CI/CD pipeline

- [ ] **Task 20: Setup Schema Change Review Process** (TECH-001 Mitigation)
  - [ ] Create `.github/CODEOWNERS` for schema files
  - [ ] Require architect approval for schema changes
  - [ ] Document schema change workflow
  - [ ] Add schema review checklist to CONTRIBUTING.md

- [ ] **Task 21: Create Data Integrity Constraint Tests** (DATA-001 Mitigation)
  - [ ] Create `database/tests/constraints.test.ts`
  - [ ] Test all unique constraints (try to violate each one)
  - [ ] Test all foreign key constraints (try to create orphans)
  - [ ] Test cascade delete behaviors
  - [ ] Test data validation rules
  - [ ] Achieve 100% constraint test coverage

- [ ] **Task 22: Create Migration Testing Script** (OPS-001 Mitigation)
  - [ ] Create `scripts/test-migration.sh`
  - [ ] Add automatic backup before migration
  - [ ] Add automatic rollback on failure
  - [ ] Add schema drift detection
  - [ ] Make executable and test with sample migration

- [ ] **Task 23: Create Database Migration Guide** (OPS-001 Mitigation)
  ...
  - [ ] Document migration safety rules
  - [ ] Include pre-deployment checklist

- [ ] **Task 24: Add Missing Index and Query Performance Testing** (PERF-001 Mitigation)
  - [ ] Add index to `replies.platform_post_id` for lookups
  - [ ] Run `EXPLAIN ANALYZE` on all core queries (authors, posts, decisions, replies)
  - [ ] Verify all frequently queried fields have indexes
  - [ ] Document query optimization strategy in `docs/DATABASE_QUERY_OPTIMIZATION.md`
  - [ ] Add to CONTRIBUTING.md: Requirement for EXPLAIN ANALYZE in PRs

- [ ] **Task 25: Create SQL Review Checklist** (DATA-001, OPS-001 Mitigation)
  - [ ] Create migration review checklist template
  - [ ] Review each migration SQL before applying
  - [ ] Verify: No DROP COLUMN commands
  - [ ] Verify: All new columns nullable or have defaults
  - [ ] Verify: Indexes named consistently
  - [ ] Verify: Foreign keys have proper ON DELETE behavior

- [ ] **Task 26: Test Complete Migration Flow** (ALL RISKS)
  - [ ] Run `pnpm validate:schema` to verify schema
  - [ ] Run `./scripts/test-migration.sh` to test migration
  - [ ] Run constraint tests: `pnpm test database/tests/constraints.test.ts`
  - [ ] Verify secret scanner catches DATABASE_URL in code
  - [ ] Test database connection with secure wrapper
  - [ ] Verify restricted database user works
...
1. **Adopt a Hybrid ENUM Strategy**: Use native Prisma `enum`s only for truly static, application-level states (`Platform`, `ApprovalStatus`, `OperationalMode`). For business logic concepts that might evolve, use a related database table instead.
   ...
   * **Action**: This refactoring pattern must be applied to:
     * `Archetype` → Table (allows dashboard to add new archetypes)
     * `PowerTier` → Table (allows adding tiers like "NANO" or "GIGA")
     * `CompetitorCategory` → Table (new competitor types without migration)
     * `KpiType` → Table (new KPIs without code changes)
     * `EscalationReason` → Table (new escalation types)
