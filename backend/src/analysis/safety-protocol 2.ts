...
export class SafetyProtocol {
  ...
  async checkSafetyProtocol(content: string, author?: Author, postId?: string): Promise<SafetySignal> {
    try {
      ...
      const contextualChecks = checks.filter(
        (check) => check.severity === SafetySeverity.HIGH || check.severity === SafetySeverity.MEDIUM,
      );
      ...
      if (contextualChecks.length > 0) {
        ...
        const severity = contextualChecks.sort(compareSeverity)[0]?.severity ?? SafetySeverity.MEDIUM;
