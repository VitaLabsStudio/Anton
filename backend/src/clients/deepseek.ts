...
export class DeepSeekClient {
  ...
  constructor(options?: DeepSeekClientOptions) {
    ...
    this.systemPrompt =
      ...
      'You are a precise text classifier. Respond with structured JSON only.';

    if (!this.apiKey) {
      logger.warn('DEEPSEEK_API_KEY environment variable is missing. DeepSeek calls will fail.');
    }
  }
  ...
  private extractConfidence(message: { content: string }): number {
    try {
      ...
      return typeof parsed.confidence === 'number' ? parsed.confidence : 0.85;
    } catch {
      return 0.85;
    }
  }
